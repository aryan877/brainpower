...
export async function getTokenPriceData(
  ...
  try {
    const url = process.env.COINGECKO_PRO_API_KEY
      ? `https://pro-api.coingecko.com/api/v3/simple/token_price/solana?contract_addresses=${tokenAddresses.join(",")}&vs_currencies=usd&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true&include_last_updated_at=true&x_cg_pro_api_key=${process.env.COINGECKO_PRO_API_KEY}`
      : `https://api.coingecko.com/api/v3/simple/token_price/solana?contract_addresses=${tokenAddresses.join(",")}&vs_currencies=usd&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true&include_last_updated_at=true${process.env.COINGECKO_DEMO_API_KEY && `&x_cg_demo_api_key=${process.env.COINGECKO_DEMO_API_KEY}`}`;
